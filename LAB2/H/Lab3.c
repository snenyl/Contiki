...
struct tempData myTemp[MAX_INFO_PER_NEIGHBOR];
uint8_t ite = 0;
...
PROCESS_THREAD(temp_process, ev, data)
{
  ...
  while(1) {
    ...
    if(myTemp[0].tempint == 0){

       myTemp[0].minus = minus;
       myTemp[0].tempfrac = tempfrac;
       myTemp[0].tempint = tempint;
  ...
  if(msg->type == UNICAST_TYPE_PING) {
    ...
    for(ii = 0; ii < numbeOfNeighbors; ii++){
        if (linkaddr_cmp(&my_neighbors[ii].addr, from)){
            ...
            my_neighbors[ii].data[a].minus = msg->td.minus;
            my_neighbors[ii].data[a].tempfrac = msg->td.tempfrac;
            my_neighbors[ii].data[a].tempint = msg->td.tempint;
            break;
        }
    }
    ...
    uint16_t sum = 0;
    for(ii = 0; ii < numbeOfNeighbors; ii++){
        if(my_neighbors[ii].data[ite].tempint == 0){
            flag = 1;
            break;
        }
        sum += my_neighbors[ii].data[ite].tempint;
    }
    
    
    if(flag == 0){
        myTemp[ite+1].tempint = (myTemp[ite].tempint * (MAX_INFO_PER_NEIGHBOR - numbeOfNeighbors) + sum)/MAX_INFO_PER_NEIGHBOR;
        ite++;
    }


    msg->type = UNICAST_TYPE_PONG;
    msg->td.minus = myTemp[ite].minus;
    msg->td.tempfrac = myTemp[ite].tempfrac;
    msg->td.tempint = myTemp[ite].tempint;
